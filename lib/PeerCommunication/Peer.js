...
module.exports = class Peer extends EventEmitter {
        ...
        switch (this.#state) {
            ...
            case PeerState.BITFIELD:
                if (messageId === 5) {
                    ...
                    //     for (let j = 0; j < 8; j++) {
                    //         if (payload[i] & (1 << (7 - j))) {
                    //             console.log('Peer has piece:', i * 8 + j);
                    //         }
                    //     }
                    // }
            ...
            case PeerState.INTERESTED:
                ...
                break;
            case PeerState.UNCHOKE:
                console.log('Peer is unchoked');
                break;
    ...
    #sendRequestMessage(payload, pieceIndex, pieceLength) {
        ...
        const payloadToSent = new Uint32Array([
            pieceIndex,
            byteOffset,
            blockSize
        ]);
        ...
        const requestMessage = Buffer.concat([Buffer.from(header.buffer),
